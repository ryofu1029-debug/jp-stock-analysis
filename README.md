...
| `search.py` | 銘柄検索。表記ゆれ（全角半角・ひらがなカタカナ・中黒・長音）を吸収したAND検索 |
...
| `tests/test_chart_extract.py` | 合成チャート画像を生成して `chart_extract.py` の抽出精度を測る |
...
サイドバーの「🔍 検索」にコード・社名・業種を入れると絞り込める（例: `8022` /
`みずの` / `銀行 セブン`）。空白区切りで複数語を入れるとAND検索。全角半角・
ひらがなカタカナ・中黒・長音記号の違いは吸収するが、**漢字社名の読み仮名は
持っていない**ので「みつびし」では引けない（「三菱」と入力する）。
...
画像からの読み取りは、出来高やRSIのサブパネルを外して価格パネルだけを検出し、
グリッド線・凡例の文字を除いたうえで動的計画法で曲線を追う。読み取り結果は
元画像に重ねて表示されるので、赤線が元チャートをなぞれているか目視で確認する
こと。外れたときは「範囲を手動で指定」でチャート本体だけを囲う。

精度の確認: `python tests/test_chart_extract.py`（合成チャート8パターンで相関を測定）
